...
export default function AdminFeatureRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const queryClient = useQueryClient();
  const [commentBody, setCommentBody] = useState('');

  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-feature-request', id],
    queryFn: () => api.get<APIResponse<FeatureRequestDetail>>(`/admin/feature-requests/${id}`),
  });
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (res) => {
      queryClient.invalidateQueries({ queryKey: ['admin-feature-request', id] });
  ...
  if (isLoading) {
    return (
      <div style={{ padding: 24, maxWidth: 820, margin: '0 auto' }}>
        <Link href="/admin/feature-requests" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: 'var(--crm-text-3)', fontSize: 13, textDecoration: 'none', marginBottom: 16 }}>
    ...
    <div style={{ padding: 24, maxWidth: 820, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="crm-card" style={{ padding: 20 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          {/* Vote count display only — admins can see but don't vote here */}
          <div
            ...
            title="Total votes (read-only on admin view)"
          >
      ...
      {/* Comments — platform admin can post replies; they'll be attributed
          to "Sarang at Tourify" so it's clear when the platform team responds. */}
      <div className="crm-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--crm-hairline)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <MessageSquare size={14} style={{ color: 'var(--crm-text-3)' }} />
        ...
        <div style={{ padding: '4px 18px' }}>
          {comments.length === 0 ? (
            <div style={{ padding: '20px 0', textAlign: 'center' }}>
              <span className="crm-dim" style={{ fontSize: 13 }}>No comments yet.</span>
            </div>
          ) : (
            <div>
              {comments.map((c, i) => (
